...
筑审智核（BuildCheck-Agent）是面向建筑施工方案的智能处理工具。本部署包含经过项目内部资格化验证的三个规范审查案例，并新增一个受控合成的跨章节内部一致性演示案例。所有案例均为局部、证据约束的机器审查结果，不代表整份方案的合规结论、安全批准或完整审查覆盖。

## 最终竞赛部署版本

本竞赛部署基于源代码版本 `9b2575b26c1654730eebb05a4596f025218629f6` 完成 D9 重构。F1 保护状态和竞赛运行时均为 `READY`，隔离 fresh-runtime 验证为 `PASS`；当前资格化的 GB 55023-2022 规范语料包含 62 条条文。
...
上述 V0.2 历史能力边界保持不变。竞赛部署在此基础上展示三个经过资格化验证、具有明确证据链的规范审查案例，并新增一个受控合成的跨章节内部一致性演示案例；系统不据此生成整份方案的合规结论、风险评级或官方批准。
...
- **Case D（`CONTROLLED_SYNTHETIC` / 受控合成演示案例）**：用途为 `INTERNAL_CONSISTENCY_DEMONSTRATION`，演示同一对象、同一参数在不同页面出现不同显式值时的候选发现。
- 受控合成文档分别给出 `30 mm` 和 `20 mm`，输出状态为 `NEEDS_HUMAN_REVIEW`。
- 该候选不代表真实项目冲突或已确认工程缺陷，不构成合规或不合规结论，也不构成严重度或风险结论；系统不据此保证整份方案内部一致，也不判断工程正确性。
- 案例执行接口：`POST /api/v1/competition/demo/cases/CASE-D/run`
